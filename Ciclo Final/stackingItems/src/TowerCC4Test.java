...
public class TowerCC4Test
{
    ...
    public void testQueFearfulLidPuedeQuitarseSiNoEstaTapandoSuTaza()
    {
        ...
        // buscar el indice de la fearful lid y quitarla con removeLid
        String[][] items = tower.stackingItem();
        int fearfulIndex = -1;
        for(int i = 0; i < items.length; i++) {
            if(items[i][0].equals("lid") && items[i][1].equals("2")) {
                fearfulIndex = i;
            }
        }
        tower.removeLid(fearfulIndex);
        assertTrue(tower.ok());
    }
    ...
    @Test
    public void testQueListedCupsContabilizaCorrectamenteConNuevosTipos()
    {
        tower.pushCup("normal", 1);
        tower.pushCup("opener", 2);
        tower.pushCup("hierarchical", 3);
        ...
        int[] tapadas = tower.listedCups();
        assertEquals(2, tapadas.length);
        assertEquals(1, tapadas[0]);
        assertEquals(2, tapadas[1]);
    }
    
    @Test
    public void testQueListedCupsRetornaNumerosDeTazasTapadasCorrectamente()
    {
        tower.pushCup("normal", 1);
        tower.pushCup("normal", 3);
