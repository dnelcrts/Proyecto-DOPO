    ...
    public ArrayList<StackingItem> swap(ArrayList<StackingItem> items, String[] o1, String[] o2)
    {
        ...
        if(o1 != null && o2 != null && o1.length >= 2 && o2.length >= 2) {
            ...
            if(i1 >= 0 && i2 >= 0 && i1 != i2) {
                ...
                // Si la taza movida a i1 tiene tapa, la movemos tambien
                StackingItem moved1 = result.get(i1);
                if(moved1 instanceof Cup && ((Cup) moved1).hasLid()) {
                    Lid lid = ((Cup) moved1).getLid();
                    int lidIndex = result.indexOf(lid);
                    result.remove(lidIndex);
                    if(lidIndex < i2) i2--;
                    result.add(i1 + 1, lid);
                    if(i1 + 1 <= i2) i2++;
                }

             // Si la taza movida a i2 tiene tapa, la movemos tambien
                StackingItem moved2 = result.get(i2);
                if(moved2 instanceof Cup && ((Cup) moved2).hasLid()) {
                    Lid lid = ((Cup) moved2).getLid();
                    int lidIndex2 = result.indexOf(lid);
                    result.remove(lidIndex2);
                    int newI2 = result.indexOf(moved2);
                    result.add(newI2 + 1, lid);
                }
            }
        }
